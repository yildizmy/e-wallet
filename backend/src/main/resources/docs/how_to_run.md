...
5. After database container starts on Docker, open backend project in using `IntelliJ IDEA` (open `e-wallet\backend` folder instead of `e-wallet`). Then select `Java 17` version via `File > Project Structure > Project > SDK` menu and run the application.
